...
void BFS() {
	queue<pair<pair<Point, Point>, pair<int, vector<char>>>> q;
	check[R.first][R.second][B.first][B.second] = true;
	q.push({ {R,B},{0 ,{}} });
	while (q.size()) {
		Point cur_R = q.front().first.first;
		Point cur_B = q.front().first.second;
		int cur_cnt = q.front().second.first;
		vector<char> cur_path = q.front().second.second;
		q.pop();
		if (cur_cnt == 10) {
			cout << -1;
			return;
		...
		for (int i = 0; i < 4; i++) {
			...
			vector<char> next_path = cur_path;
			next_path.emplace_back(path[i]);
			if (next_B.first == Hole)	continue;
			if (next_R.first == Hole) {
				cout << cur_cnt + 1 << "\n";
				for (auto it : next_path) {
					cout << it;
				}
				return;
			}
			...
			if (!check[next_R.first.first][next_R.first.second][next_B.first.first][next_B.first.second]) {
				q.push({ {next_R.first,next_B.first},{cur_cnt + 1,next_path} });
